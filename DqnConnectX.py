...
class Network(nn.Module):
    ...
    def __init__(self, input_size, nb_action):
        ...
        # Hidden layer with 30 hidden units
        self.fc1 = nn.Linear(input_size, 30)
        # Output layer
        self.fc2 = nn.Linear(30, nb_action)
    
    def forward(self, state):
        # Get Hidden Layer Values by passing through
        # fc1 and relu activation
        x = F.relu(self.fc1(state))
        # Get Output layer Q values
        q_values = self.fc2(x)
        return q_values
...
class ReplayMemory(object):
    ...
    def sample(self, batch_size):
        ...
        # that can be used.
        # Create an iterator of Variables that can be passed in. 
